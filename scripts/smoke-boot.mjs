...
//      bundles entry spliced, the patch layer dropped)
//
// This is the gate the unit suite cannot provide: it proves the plugin's
// `inject: ['commands', 'userQuestions']` seam matches the real host and
// that apply() registers the commands against a live commands service. Unit
// tests + typecheck run against linked type packages; only this catches a
// seam drift on boot.
//
...
const ownName = pkg.name // @aiwayds/dsh-profile-switch

const work = mkdtempSync(path.join(tmpdir(), 'dsh-profile-switch-smoke-'))
...
if (dumpAfter.stdout.includes('dsh-profile-switch')) {
