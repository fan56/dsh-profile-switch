 ...
 * be saved, configured and applied in one step, so switching between
 * contexts (work ↔ personal) is a single command instead of a /model +
 * per-agent tour.
 *
 * This is THE write side of the store (moved here from dsh-tui-pi's
 * profile feature); dsh-subagent-registry is the read side: it composes
 * each agent spawn's runtime model/thinking as
 *
 *   effective = profile override ⊕ frontmatter baseline
 *
 * per workspace pin, re-read on every dispatch. Storage:
 * `$DSH_HOME/model-profiles.json` — a small versioned JSON document this
 * plugin owns outright. Reads never throw: a missing, corrupt or
 ...
 *   pinned profile's overrides (the registry's composeAgentRuntime, per
 *   workspace pin) — which is what keeps a switch workspace-scoped. Agents
 *   absent from the profile compose from the baseline.
...
import { dshHome } from './agents-dir.ts'
...
export interface ModelProfilesDoc {
  /** Informational: the profile the last switch applied, anywhere. No UI
   * reads this — the ● "current" markers show the tree's own binding
...
/** The plain-agent shape the snapshot capture consumes (frontmatter baseline). */
 ...
 * `undefined` on success — callers surface it in the summary, never throw.
 */
 ...
 */
export function captureAgentsSnapshot(agents: readonly AgentBaseline[]): Record<string, ProfileAgentEntry> {
 ...
 * and staying uncached means a hand-added or removed pin takes effect on the
 * very next session in this process. Returns `undefined` when nothing is
 * found.
 */
...
/**
 * Write `<dir>/.dsh-profile` naming `profileName`. Resolves an error
 * message on failure, else `undefined`.
 */
 ...
 * not of the machine, so the global `doc.current` never drives any UI.
 */
 ...
 * refused so unbinding can never silently destroy manual edits. Resolves an
 * error message on refusal or fs failure, else `undefined`.
 */
