 ...
 * behind /profile-switch + /profile-cfg.
 *
 * Contract under test:
 ...
 *   compose path lives in dsh-subagent-registry (the read side); the pure
 *   store layer plans no agent-file writes.
...
function tempStore() {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-test-'))
  return {
    dir,
...
/** Minimal agent baseline — only name/model/thinking matter here. */
function agentBaseline(name, model, thinking) {
  return {
    name,
...
test('saveModelProfiles creates the directory when missing and leaves no tmp sibling', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-nested-'))
...
test('saveModelProfiles resolves an error message instead of throwing on failure', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-fail-'))
...
test('captureAgentsSnapshot records overrides and keeps inherit agents as empty entries', () => {
  const snapshot = captureAgentsSnapshot([
    agentBaseline('workhorse', 'p/m', 'high'),
    agentBaseline('duck'),
  ])
...
test('a captured snapshot saved and reloaded keeps the recorded agents', () => {
  ...
  try {
    ...
    profile.agents = captureAgentsSnapshot([agentBaseline('workhorse', 'a/b', 'high'), agentBaseline('duck')])
...
test('writeProfilePin + readNearestProfilePin: nearest file up the tree wins', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-'))
...
test('readNearestProfilePin tolerates an unreadable or garbage pin file', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-bad-'))
  try {
...
test('resolvePinnedProfile returns the named profile, undefined when unknown/unbound', () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-res-'))
  try {
...
test('removeProfilePin removes a simple matching file and refuses everything else', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-rm-'))
...
test('bindWorkspaceProfile writes a fresh file and rebinds a clean one', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-bind-'))
  const path = join(dir, PROFILE_PIN_FILE)
  try {
...
test('bindWorkspaceProfile refuses a hand-decorated file and leaves it untouched', () => {
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-bind2-'))
  const path = join(dir, PROFILE_PIN_FILE)
  try {
...
test('boundProfileName resolves the nearest binding through the doc, undefined when unknown', () => {
  const doc = seedModelProfilesDoc()
  const dir = mkdtempSync(join(tmpdir(), 'dsh-profile-switch-pin-cur-'))
